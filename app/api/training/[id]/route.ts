...
export async function GET(
  ...
  try {
    ...
    const training = await prisma.trainingCourse.findUnique({
      where: { id },
    })
    ...
    return NextResponse.json(
      { error: "Failed to fetch training course" },
      { status: 500 }
    )
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: "Training course deleted successfully" })
  } catch (error) {
    return NextResponse.json(
